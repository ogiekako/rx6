...
やはりあいかわらず stack が壊れてしまう。userinit まではたどり着いていて、その中の stosl でやはり落ちている。これは、stosl が stack を壊しているのか？ でも場所はちがっても同じく stosl で壊れるというのは、それが間違っているんだよなあ。x86 の出力と比べてみよう。

stosl がどんな命令で、それでなにを実現しようとしているかを理解する。
memset 内で、stosl は使われていて、指定したアドレスを指定した長さで 0 クリアするときなどに使われている。長さの指定を間違えると、別のデータを破壊する可能性がある。
